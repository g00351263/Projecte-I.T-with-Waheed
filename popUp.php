...
<?php
	...
	// php script for showing app the add after it was clicked on index or related category page //
	$conn = @mysql_connect('localhost','root',''); // connections to database //
	$db   = mysql_select_db('registration', $conn);
	
	// ad was clicked it will trigger the href of name pop in index pages //
	if(isset($_GET['pop']))
	{
		$id = $_GET['pop']; // getting the id value from another page into variable //
		$sql = "SELECT * FROM ads WHERE id='$id'"; // sql query //
		$result = mysql_query($sql) or die("Failed".mysql_error());
		$row = mysql_fetch_array($result);
		
		// below are all values in ad table //
		...
		// below will be printed in HTML page //
	echo ' 
